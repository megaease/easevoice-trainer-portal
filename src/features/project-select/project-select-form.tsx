...
export default function ProjectSelectForm() {
  ...
  const { data: namespacesData, isLoading, refetch } = useQuery({
    queryKey: ['namespaces'],
    queryFn: async () => {
  ...
  const createDefaultNamespace = useMutation({
    ...
    onError: (error) => {
      toast.error(`创建默认项目失败: ${error instanceof Error ? error.message : '未知错误'}`)
    }
  })

  
  useEffect(() => {
    if (namespacesData && namespacesData.namespaces && namespacesData.namespaces.length === 0) {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>克隆和训练模型产生的文件都会保存在项目中</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
